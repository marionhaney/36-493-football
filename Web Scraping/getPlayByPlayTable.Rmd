...
```{r}
getPlayByPlayTable = function(links, wantedTeam = "Carnegie Mellon") {
  bigBadTable = NULL
  for (series in links) {
    ...
    homeTeamScore = homeInfo[3]
        ...
        # only wanted teams
    if ((!(homeTeam == wantedTeam) && !(awayTeam == wantedTeam))) {
      next
    }
...
df = getPlayByPlayTable(playByPlayLinks, wantedTeam = "Carnegie Mellon")
```
