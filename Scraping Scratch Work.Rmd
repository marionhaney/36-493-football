...
```{r}
makeWeekTable = function(links, bigBadTable, wantedTeams = c("Carnegie Mellon")) {
...
```{r}
getConferenceTeams = function(conference = "PAC") {
  # Vector of teams in conference
  confTeams = teams %>% subset(., Conference == conference, select = School) %>% unlist(use.names = FALSE)
  return(confTeams)
}
```

```{r}
getRegionTeams = function(region = 2) {
  # Vector of teams in region
  regionalTeams = teams %>% subset(., Region == region, select = School) %>% unlist(use.names = FALSE)
  return(regionalTeams)
}
```

```{r}
getSurfaceTeams = function(surface = "Turf") {
  # Vector of teams by field surface type
  surfaceTeams = teams %>% subset(., Surface == surface, select = School) %>% unlist(use.names = FALSE)
  return(surfaceTeams)
}
```

```{r}
getNicknameTeam = function(nickname = "Tartans") {
  # Vector of teams with nickname
  nicknameTeams = teams %>% subset(., Nickname %in% nickname, select = School) %>% unlist(use.names = FALSE)
