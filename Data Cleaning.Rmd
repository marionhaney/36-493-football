...
drive.data %>% filter(drive.outcome=="FG" | drive.outcome=="TD For" | drive.outcome=="TD Against") %>%
  ggplot(aes(x=yds.to.endzone)) + geom_histogram(bins=25) + 
  labs(title="Starting location of drives that end in a scoring event", x="Yards to endzone")
