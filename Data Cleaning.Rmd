...
for (i in 1:num.games) { # Go through every game
  ...
  for (j in 1:num.plays) { # Go through every play in the game
    ...
    if ((cur.play.type %in% drive.preceders) &&
        (j != 1) &&
        !(ind.game[j - 1, "play.type"] %in% drive.enders)) { #Halftime kickoff
      drive = drive + 1
    }
    else if ((cur.result %in% drive.starters) && 
             !(cur.play.type %in% drive.enders) &&
             (j != 1) &&
             (ind.game[j, "odk"] != ind.game[j + 1, "odk"])) { #Turnover
      drive = drive + 1
    }
    
    drive.col[j] = drive
    
    if ((cur.play.type %in% drive.enders) ||
        ((j != 1) && # Not opening kickoff
         (j != num.plays) &&  # Don't want index out of bounds
         !(cur.result %in% drive.starters) && # Don't want it to have entered previous if statement
         (ind.game[j, "odk"] %in% od) && # Current play must be offense or defense
         (ind.game[j + 1, "odk"] %in% od) && # Next play must be offense or defense
         (ind.game[j, "odk"] != ind.game[j + 1, "odk"]))) { #Rest of drive changers
                                                            #OR condition checks for turnover on downs
      drive = drive + 1
    }
  }
...
if (nrow(init.data)==2083) { init.data = init.data[-2083,] }
last.plays = c(which(init.data$drive!=lead(init.data$drive)), nrow(init.data))
last.results = init.data$result[last.plays]
last.results = ifelse(is.na(init.data$result[last.plays]), init.data$play.type[last.plays],
                      init.data$result[last.plays])
which(is.na(init.data$play.type) & is.na(init.data$result))
