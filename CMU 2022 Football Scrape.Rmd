...
print(seasonSchedule(year = 2021))
```


Get the main URL of all games during WK and season YEAR: (commented out because made a function)
```{r}
# # url to all D3 games of particular year in particular week
# 
# url = "https://www.d3football.com/scoreboard/2022/composite?view=5"
# 
...
Get links to play by play lines: (commented out because made a function)
```{r}
...
gamePlayByPlay = function(game, date, time, location, homeTeam, homeAbb, awayTeam, awayAbb) {
  ...
  for (i in 1:nrow(game)) {
    ...
    if (nchar(situation) >= 25 && startsWith %in% downs) {
      ...
      if (!(is.na(str_extract(play, "(kickoff|hurry|ball|drive|quarter|half|Timeout|game|toss|QB|receive)")))) {
        # has no wanted information, so skip
        next
      }
      ...
      # get play
      currentPlay = str_extract(play, "(rush|pass|PENALTY|fumble|sacked|punt|field goal|kick|Kneel)")
...
Create the table: (commented out because made a function)
```{r}
# 
# bigBadTable = NULL
# teamScrapeDone = FALSE
# for (series in links) {
#   # if we already did the CMU scrape, no need to continue
...
for (m in 1:length(listOfUrls)) {
    ...
    teamScrapeDone = FALSE
    for (series in links) {
      # if we already did the team scrape, no need to continue
      if (teamScrapeDone) { break }
      # play by play of game
      gameSeries = read_html(series, as.data.frame = TRUE, stringsAsFactors = TRUE)
      y = gameSeries %>% html_nodes("table") %>% .[[3]]
...
```{r}
# urls for the 2022 season, weeks 1-10
URLS = c("https://www.d3football.com/scoreboard/2022/composite?view=1", 
        "https://www.d3football.com/scoreboard/2022/composite?view=2",
        "https://www.d3football.com/scoreboard/2022/composite?view=3",
        "https://www.d3football.com/scoreboard/2022/composite?view=4",
        "https://www.d3football.com/scoreboard/2022/composite?view=5",
        "https://www.d3football.com/scoreboard/2022/composite?view=6",
        "https://www.d3football.com/scoreboard/2022/composite?view=7", 
        "https://www.d3football.com/scoreboard/2022/composite?view=8", 
        "https://www.d3football.com/scoreboard/2022/composite?view=9", 
        "https://www.d3football.com/scoreboard/2022/composite?view=10")
```
...
batchScrape = function(urls, teams) {
  ...
  for (i in 1:length(teams)) {
    team = teams[i]
    print(team)
    newDf = combineSeasonData(urls, team)
    df = rbind(df, newDf)
    teamsSoFar = append(teamsSoFar, team)
  }
...
# make df
#df = combineSeasonData(URLS, "Carnegie Mellon")
teams = c("Carnegie Mellon", "Whitworth")
checkURLs = c("https://www.d3football.com/scoreboard/2022/composite?view=1", 
        "https://www.d3football.com/scoreboard/2022/composite?view=2")
df = batchScrape(checkURLs, teams)
