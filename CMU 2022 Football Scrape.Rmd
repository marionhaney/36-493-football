...
gamePlayByPlay = function(game, date, time, location, homeTeam, homeAbb, awayTeam, awayAbb) {
  ...
  for (i in 1:nrow(game)) {
    ...
    if (nchar(situation) >= 25 && startsWith %in% downs) {
      ...
      if (is.null(currentPossession) || length(currentPossession == 0) || is.na(currentPossession)) {
        # no possession, coin toss or weird row
        next
      } 
...
#df = data.frame(bigBadTable) # run initially then comment out
#bigBadTable["Home.Team.Abbreviation"] = "RPI"
#df = rbind(df, data.frame(bigBadTable)) # run for each week
```
